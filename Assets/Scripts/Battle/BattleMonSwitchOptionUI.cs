...
public class BattleMonSwitchOptionUI : MonoBehaviour
{
    ...
    public void Bind(MonInstance mon, Action<MonInstance> callback)
    {
        ...
        if (monSprite != null)
        {
            monSprite.sprite = boundMon.species.backSprite;
        }

        if (typeIcon != null)
        {
            typeIcon.sprite = boundMon.species.typeSprite;
        }

        if (monName != null)
        {
            monName.text = boundMon.species.monName;
        }

        if (monLevel != null)
        {
            monLevel.text = $"Lvl {boundMon.level}";
        }
