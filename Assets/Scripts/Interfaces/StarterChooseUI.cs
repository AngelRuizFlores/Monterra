...
public class StarterChooseUI : MonoBehaviour
{
    ...
    private void OnEnable()
    {
        ...
        if (allStarters == null || allStarters.Count < 3)
        {
            return;
        }
    ...
    {
        List<MonSpecies> temp = new List<MonSpecies>(allStarters);
    ...
    private void Paint()
    {
        for (int i = 0; i < 3; i++)
        {
            ...
            {
                slotImages[i].sprite = species.frontSprite;
            }

            if (slotNames != null && i < slotNames.Length && slotNames[i] != null)
            {
                slotNames[i].text = species.monName;
            }
            ...
            TMP_Text text = slotButtons[i].GetComponentInChildren<TMP_Text>();

            if (text != null)
            {
                text.text = species.type + " Type";
            }

            Color typeColor = GetTypeColor(species.type);
    ...
    private void HookButtons()
    {
        for (int i = 0; i < 3; i++)
        {
            ...
            slotButtons[i].onClick.AddListener(() => Choose(index));
        }
    }
    ...
    private IEnumerator ChooseRoutine(MonSpecies chosen)
    {
        ...
        MonInstance starter = new MonInstance
        {
            species = chosen,
            level = 3,
            experience = 0
