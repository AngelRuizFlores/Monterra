...
public sealed class EndGameSequenceController : MonoBehaviour
{
    ...
    private void DisableGameplayInteractions()
    {
        TouchingBehaviour touching = FindFirstObjectByType<TouchingBehaviour>();

        if (touching != null)
        {
            touching.enabled = false;
        }

        MovementBehaviour movement = FindFirstObjectByType<MovementBehaviour>();
        ...
        TeamKeyboardInput teamKeyboardInput = FindFirstObjectByType<TeamKeyboardInput>();

        if (teamKeyboardInput != null)
        {
            teamKeyboardInput.enabled = false;
        }
    }
}